...
class AppSettingsView(APIView):
    ...
    def get(self, request):
        # Business/marketing info resolves in two layers (core.whitelabel):
        # MOMENTI_* env defaults, then admin overrides from the White-label
        # tab in /admin. Blank values are omitted so the UI hides the empty
        # blocks rather than showing placeholders.
        from . import whitelabel

        merged = whitelabel.resolved()
        ...
        return Response(
            {
                "id": "local",
                "public_settings": {
                    "app_name": business.get("name") or "momenti.co",
                    "auth_mode": "password",
                    "business": business,
                    "branding": branding,
                },
